...
#We filter only the design we desire:
filtered_design <- subset(Exp.design,
                          policy.name == "Nordhaus+R&DGreenClimateFund" &
                            ((abs(epsilon - 0.0453) < 1e-6 & abs(rho - 0.1477) < 1e-6) |
                               (abs(epsilon - 0.0453) < 1e-6 & abs(rho - 0.6477) < 1e-6) |
                               (abs(epsilon - 0.0978) < 1e-6 & abs(rho - 0.6477) < 1e-6)))
Exp.design <- filtered_design
return(Exp.design)
}
